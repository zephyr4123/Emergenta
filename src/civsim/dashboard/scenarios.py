"""场景预设模块 — 荷兰病、信息茧房、世界末日一键配置。

将 scripts/ 下三大经典场景的核心设定封装为运行时可注入的预设，
通过造物主面板一键应用到正在运行的仿真引擎。
"""

from __future__ import annotations

import random
from dataclasses import dataclass
from typing import Any

from civsim.world.tiles import TileType


@dataclass
class ScenarioPreset:
    """场景预设元数据。

    Attributes:
        key: 场景标识符。
        name: 中文名称。
        description: 场景简述。
    """

    key: str
    name: str
    description: str


# ── 预设注册表 ───────────────────────────────────────────────

SCENARIO_REGISTRY: list[ScenarioPreset] = [
    ScenarioPreset(
        key="dutch_disease",
        name="荷兰病（资源诅咒）",
        description="一个聚落黄金暴富但粮食为零、农田荒废，观察贸易依赖与社会崩溃",
    ),
    ScenarioPreset(
        key="info_cocoon",
        name="信息茧房（虚假繁荣）",
        description="~25%聚落处于高税低粮极端困境，其余正常，观察涌现差异",
    ),
    ScenarioPreset(
        key="apocalypse",
        name="世界末日（全球危机）",
        description="所有聚落同时陷入极端资源匮乏+70%农田毁坏，观察幸存率",
    ),
]


def get_preset_options() -> list[dict[str, str]]:
    """返回场景预设下拉框选项列表。"""
    return [
        {"label": p.name, "value": p.key}
        for p in SCENARIO_REGISTRY
    ]


# ── 场景应用函数 ─────────────────────────────────────────────


def apply_scenario(engine: Any, key: str) -> list[str]:
    """将指定场景预设应用到运行中的引擎。

    Args:
        engine: CivilizationEngine 实例。
        key: 场景标识符。

    Returns:
        事件日志消息列表。
    """
    handler = _SCENARIO_HANDLERS.get(key)
    if handler is None:
        return [f"⚠ 未知场景: {key}"]
    return handler(engine)


# ── 场景 Prompt 注入 ─────────────────────────────────────────


def _inject_scenario_prompts(
    engine: Any,
    scenario_desc: str,
) -> str:
    """将场景背景信息注入所有 LLM Agent 的 system prompt。

    Args:
        engine: CivilizationEngine 实例。
        scenario_desc: 场景背景描述（中文）。

    Returns:
        日志消息。
    """
    gov_count = 0
    leader_count = 0

    gov_prompt = (
        "你是一个文明模拟器中的聚落镇长AI。你有自己的执政风格。\n"
        "你可能贪婪、铁腕、仁慈或投机。\n"
        "你的首要目标是让你管辖的聚落在与其他聚落的竞争中存活并壮大。\n\n"
        f"【当前场景】{scenario_desc}\n"
        "你应当根据此场景背景，结合当前聚落数据做出决策。\n\n"
        "决策原则：\n"
        "1. 当资源紧张时，你可以选择囤积不贸易，甚至提高税率以应急\n"
        "2. 当其他聚落虚弱时，你可以建议首领发动战争掠夺\n"
        "3. 治安投入有代价——过高的治安会引发民众更强烈的反感\n"
        "4. 单次税率调整幅度不应超过 0.1\n"
        "5. 治安投入变化幅度不应超过 0.15\n"
        "6. 不要总是降税和提高治安，这种策略长期不可持续\n"
    )

    leader_prompt = (
        "你是一个文明模拟器中的阵营首领AI。你统领多个聚落。\n"
        "你的核心目标是：扩张势力、获取资源、削弱对手。\n\n"
        f"【当前场景】{scenario_desc}\n"
        "你应当根据此场景背景，制定战略决策。\n\n"
        "决策范围：\n"
        "1. 外交行动：propose_alliance / propose_trade / declare_war / "
        "offer_peace / break_treaty / trade_embargo / none\n"
        "2. 政策指令：向下属聚落发布税率/治安/资源重点调整指令\n"
        "3. 军事命令：对敌对阵营发起攻击或防御\n\n"
        "决策原则：\n"
        "1. 当你总人口 > 对手 1.3 倍时，考虑宣战掠夺\n"
        "2. 当盟友变弱或威胁到你时，考虑背叛\n"
        "3. 当内部满意度低时，可以对外宣战来转嫁矛盾\n"
        "4. 联盟是暂时的工具，战争是资源再分配的手段\n"
    )

    if hasattr(engine, "get_governors"):
        for gov in engine.get_governors():
            gov.system_prompt_override = gov_prompt
            gov_count += 1

    if hasattr(engine, "leaders"):
        for leader in engine.leaders:
            leader.system_prompt_override = leader_prompt
            leader_count += 1

    return f"🧠 场景 Prompt 已注入: {gov_count} 镇长, {leader_count} 首领"


# ── 生存保障参数包（三场景共用）──────────────────────────────


def _apply_survival_params(engine: Any) -> str:
    """注入场景基础参数，让文明在压力下真正挣扎。

    核心思路：保留一线生机但不提供安全网。
    文明必须通过 LLM 决策和贸易自救，否则大量人口会死亡。

    Args:
        engine: CivilizationEngine 实例。

    Returns:
        日志消息。
    """
    c = engine.config

    # ── 食物经济：微幅调整，不提供安全网 ──
    c.civilian_behavior.work_output_food = 3.0      # 2.5→3.0（仅+20%）
    c.resources.consumption.food_per_civilian_per_tick = 0.45  # 0.5→0.45（仅-10%）

    # ── 饥荒死亡率：保持高压 ──
    c.settlement_params.starvation_unfed_factor = 0.03  # 0.05→0.03（仅-40%）
    c.engine_params.starvation_rate_factor = 0.06       # 0.1→0.06（仅-40%）

    # ── 革命惩罚：依然痛苦 ──
    c.revolution_params.resource_penalty_food = 0.6      # 0.8→0.6（食物损失60%）
    c.revolution_params.population_loss_ratio = 0.07     # 0.1→0.07
    c.revolution_params.protest_threshold = 0.25         # 0.20→0.25（稍高）

    # ── 抗议传染：保持原强度 ──
    c.markov_coefficients.hunger_to_protest_working = 0.50   # 0.60→0.50
    c.markov_coefficients.hunger_to_protest_social = 0.40    # 0.50→0.40
    c.markov_coefficients.granovetter_burst_working = 0.65   # 0.80→0.65
    c.markov_coefficients.granovetter_burst_social = 0.70    # 0.85→0.70

    # ── 贸易：适度提效 ──
    c.trade_params.food_surplus_threshold = 3.5
    c.trade_params.surplus_trade_ratio = 0.4
    c.trade_params.refuse_prob_base = 0.15
    c.trade_params.max_trades_per_settlement_per_tick = 4

    return (
        "⚙ 场景参数已注入: "
        "食物经济微调, 饥荒死亡率高, "
        "革命惩罚重, 贸易适度提效"
    )


# ── 荷兰病 ──────────────────────────────────────────────────


def _apply_dutch_disease(engine: Any) -> list[str]:
    """荷兰病：一个聚落暴富但无粮。

    富裕聚落拥有巨量黄金但农田荒废、粮食为零，
    必须依赖贸易从其他聚落购买食物才能存活。
    """
    logs: list[str] = []
    settlements = list(engine.settlements.values())
    if not settlements:
        return ["⚠ 无聚落可配置"]

    # 选人口最多的聚落作为"富裕聚落"
    rich = max(settlements, key=lambda s: s.population)
    others = [s for s in settlements if s.id != rich.id]

    # ── 注入生存保障参数 ──
    if hasattr(engine, "config"):
        logs.append(_apply_survival_params(engine))

    # ── 富裕聚落：暴金、零粮 ──
    rich.stockpile["gold"] = 50000.0
    rich.stockpile["food"] = 0.0
    rich.tax_rate = 0.1
    rich.security_level = 0.5
    logs.append(
        f"💰 [{rich.name}] 金币→50000, 食物→0, "
        f"税率→10%, 治安→50%"
    )

    # 摧毁富裕聚落领地内的农田（近乎完全荒废）
    destroyed = 0
    for tx, ty in rich.territory_tiles:
        tile = engine.tile_grid[tx][ty]
        if tile.tile_type == TileType.FARMLAND:
            tile.fertility = 0.02
            destroyed += 1
    if destroyed:
        logs.append(f"🏜 [{rich.name}] {destroyed} 块农田肥力降至 0.02")

    # ── 其余聚落：适中粮食、少量金币 ──
    for s in others:
        s.stockpile["food"] = 1500.0
        s.stockpile["gold"] = 30.0
        s.tax_rate = 0.20
        s.security_level = 0.5
    logs.append(
        f"🌾 其余 {len(others)} 个聚落: "
        f"食物→1500, 金币→30, 税率→20%"
    )

    logs.append(_inject_scenario_prompts(
        engine,
        "荷兰病（资源诅咒）— 你管辖的文明中，有一个聚落因发现大量黄金而暴富，"
        "但其农田已荒废、粮食储备为零。该聚落必须通过贸易用金币换取食物才能存活。"
        "其他聚落拥有充足的粮食但金币稀少。"
        "你需要关注贸易依赖关系、粮食安全、以及资源不平等带来的社会动荡。",
    ))
    logs.insert(0, "🎬 场景预设 [荷兰病] 已应用")
    return logs


# ── 信息茧房 ────────────────────────────────────────────────


def _apply_info_cocoon(engine: Any) -> list[str]:
    """信息茧房：部分聚落极端困境。

    ~15%聚落处于高税低粮的极端困境，其余正常。
    困境聚落面临真实生存压力，观察是否有迁徙/贸易救济。
    """
    logs: list[str] = []
    settlements = list(engine.settlements.values())
    if not settlements:
        return ["⚠ 无聚落可配置"]

    # ── 注入生存保障参数 ──
    if hasattr(engine, "config"):
        logs.append(_apply_survival_params(engine))

    # ~25% 聚落陷入困境
    n_lying = max(1, int(len(settlements) * 0.25))
    rng = random.Random(42)
    crisis = rng.sample(settlements, min(n_lying, len(settlements)))
    crisis_ids = {s.id for s in crisis}
    honest = [s for s in settlements if s.id not in crisis_ids]

    # ── 困境聚落：高税、极低粮、低治安 ──
    for s in crisis:
        s.stockpile["food"] = 30.0
        s.stockpile["gold"] = 10.0
        s.tax_rate = 0.65
        s.security_level = 0.2

    crisis_names = ", ".join(s.name for s in crisis[:5])
    suffix = f"等{len(crisis)}个" if len(crisis) > 5 else ""
    logs.append(
        f"🔴 困境聚落 [{crisis_names}{suffix}]: "
        f"食物→30, 税率→65%, 治安→20%"
    )

    # ── 正常聚落 ──
    for s in honest:
        s.stockpile["food"] = 1500.0
        s.tax_rate = 0.20
        s.security_level = 0.5
    logs.append(
        f"🟢 正常聚落 ({len(honest)}个): "
        f"食物→1500, 税率→20%, 治安→50%"
    )

    logs.append(_inject_scenario_prompts(
        engine,
        "信息茧房（虚假繁荣）— 你管辖的文明中，约15%的聚落处于极端困境"
        "（高税收、低粮食、低治安），而其余聚落相对正常。"
        "困境聚落的民众极度不满，随时可能爆发革命。"
        "你需要关注聚落间的差距、是否应该援助困境聚落、"
        "以及虚假繁荣表象下的真实危机。",
    ))
    logs.insert(0, "🎬 场景预设 [信息茧房] 已应用")
    return logs


# ── 世界末日 ────────────────────────────────────────────────


def _apply_apocalypse(engine: Any) -> list[str]:
    """世界末日：全局极端危机。

    所有聚落同时陷入资源匮乏，50%农田被毁。
    文明需要在极端逆境中缓慢恢复。
    """
    logs: list[str] = []
    settlements = list(engine.settlements.values())
    if not settlements:
        return ["⚠ 无聚落可配置"]

    # ── 注入生存保障参数 ──
    if hasattr(engine, "config"):
        logs.append(_apply_survival_params(engine))

    # ── 所有聚落：极端匮乏 ──
    for s in settlements:
        s.stockpile["food"] = 30.0
        s.stockpile["gold"] = 5.0
        s.stockpile["wood"] = 5.0
        s.stockpile["ore"] = 2.0
        s.tax_rate = 0.55
        s.security_level = 0.2
    logs.append(
        f"💀 全部 {len(settlements)} 个聚落: "
        f"食物→30, 金→5, 木→5, 矿→2, 税→55%, 治安→20%"
    )

    # ── 摧毁 70% 农田 ──
    farmland_tiles = []
    w = len(engine.tile_grid)
    h = len(engine.tile_grid[0]) if w > 0 else 0
    for x in range(w):
        for y in range(h):
            tile = engine.tile_grid[x][y]
            if tile.tile_type == TileType.FARMLAND:
                farmland_tiles.append(tile)

    rng = random.Random(77)
    n_destroy = int(len(farmland_tiles) * 0.7)
    targets = rng.sample(farmland_tiles, min(n_destroy, len(farmland_tiles)))
    for tile in targets:
        tile.fertility = 0.03
    logs.append(
        f"🏜 {len(targets)}/{len(farmland_tiles)} 块农田"
        f"肥力降至 0.03"
    )

    logs.append(_inject_scenario_prompts(
        engine,
        "世界末日（全球危机）— 所有聚落同时陷入极端资源匮乏，"
        "50%的农田被毁坏，食物、木材、矿石、金币储备都降至最低。"
        "文明正处于灭绝边缘。你需要在极端逆境中做出艰难抉择："
        "是优先保障粮食安全，还是维持治安防止暴乱，"
        "还是通过外交合作共度难关。每一个决策都关乎文明的存亡。",
    ))
    logs.insert(0, "🎬 场景预设 [世界末日] 已应用")
    return logs


# ── 场景处理器映射 ────────────────────────────────────────────

_SCENARIO_HANDLERS: dict[str, Any] = {
    "dutch_disease": _apply_dutch_disease,
    "info_cocoon": _apply_info_cocoon,
    "apocalypse": _apply_apocalypse,
}
